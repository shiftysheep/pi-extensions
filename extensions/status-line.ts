...
export default function (pi: ExtensionAPI) {
	...
	async function openaiCodexAccessToken(): Promise<string | null> {
		let auth: any;
		try { auth = JSON.parse(fs.readFileSync(codexAuthPath(), "utf8")); } catch { return null; }
		...
		const expMs = (jwtExp(tokens.access_token) ?? jwtExp(tokens.id_token)) != null
		...
		let json: any;
		try { json = await res.json(); } catch { return tokens.access_token; }
		...
		} catch { /* non-fatal: just don't persist */ }
		return freshAccess === json.id_token ? tokens.access_token : (json.access_token ?? tokens.access_token);
	}
	...
	function fetchQuota(): Promise<QuotaState | undefined> {
		...
		if (quotaCache && now - quotaCache.fetchedAt < QUOTA_CACHE_MS) return Promise.resolve(quotaCache);
		...
		const doFetch = async (): Promise<QuotaState | undefined> => {
			...
			let res: Response;
			try {
				res = await fetch("https://chatgpt.com/backend-api/wham/usage", { headers: { Authorization: `Bearer ${token}` } });
			} catch (err) { throw err; }
			if (!res.ok) throw new Error(`wham/usage HTTP ${res.status}`);
			let json: any;
			try { json = await res.json(); } catch (err) { throw err; }
	...
	function currentModelIsSubscription(): boolean {
		try {
			...
			return Boolean((lastCtx as any)?.modelRegistry?.isUsingOAuth?.(m) &&
				(lastCtx as any)?.modelRegistry?.getProvider?.(m.provider)?.auth?.oauth?.isSubscription);
		} catch { return false; /* optional provider/auth metadata */ }
	}
	...
	function formatCwdForFooter(cwd: string, home: string): string {
		...
		const isInsideHome =
			relPath === "" || (relPath !== ".." && !relPath.startsWith(`..${sep}`) && !(relPath.startsWith("/") || /^[A-Za-z]:/.test(relPath)));
		if (!isInsideHome) return cwd;
		return relPath === "" ? "~" : `~${sep}${relPath}`;
	}

	function computeTotals(sm: any): {
		input: number; output: number; cacheRead: number; cacheWrite: number; cost: number; latestCacheHitRate?: number;
	} {
		...
		for (const entry of sm.getEntries()) {
			...
			if (entry.type === "message" && msg.role === "assistant") {
				...
				latestCacheHitRate = latestPromptTokens > 0 ? ((u.cacheRead ?? 0) / latestPromptTokens) * 100 : undefined;
			} else if (entry.type === "message" && msg.role === "toolResult" && msg.usage) {
				const u: any = msg.usage;
				input += u.input ?? 0; output += u.output ?? 0; cacheRead += u.cacheRead ?? 0; cacheWrite += u.cacheWrite ?? 0; cost += u.cost?.total ?? 0;
	...
	function renderLines(width: number): string[] {
		...
		const contextUsage: any = typeof lastCtx?.getContextUsage === "function" ? lastCtx.getContextUsage() : null;
		const percentValue: number = contextUsage && contextUsage.percent != null ? contextUsage.percent : 0;
		...
		const ctxDisplay = (pct: string | undefined): string => contextUsage?.percent === null || !contextUsage
		...
		if (contextUsage && contextUsage.percent !== null) {
			...
			ctxPercentStr = percentValue > 90 ? theme.fg("error", ctxDisplay(pct)) : percentValue > 70 ? theme.fg("warning", ctxDisplay(pct)) : ctxDisplay(pct);
		} else {
		...
		let usingSubscription = currentModelIsSubscription();
		if (totals.cost || usingSubscription) statsParts.push(`$${totals.cost.toFixed(3)}${usingSubscription ? " (sub)" : ""}`);
		...
		} catch { /* accessor unavailable in this build version */ }
		...
		if (rightSide !== rightSideRaw && statsLeftWidth + minPadding + visibleWidth(rightSide) > width) {
		...
		let line1: string;
		try {
			line1 = formatCwdForFooter(sm.getCwd ? sm.getCwd() : "", process.env.HOME || (process as any).env?.USERPROFILE || homedir());
		} catch (err) { console.error("footer-cwd error", err); line1 = ""; } // should not happen, but avoid throwing out of render()
		...
		} catch (err) { console.error("footer-session-name error", err); } // ignore failures around names
		...
		} catch { /* ignore: extension statuses are optional */ }
	...
	function maybeFetchQuota(): void {
		try { if (!currentModelIsSubscription()) return; fetchQuota().then((q) => { if (q) requestRender(); }).catch(() => {}); } catch {}
	}
	...
	function installFooter(ctx: any): void {
		...
		ctx.ui.setFooter((tui: any, theme: any, footerData: any) => {
			...
			try {
				unsubBranch?.();
				if (typeof footerData?.onBranchChange === "function") unsubBranch = footerData.onBranchChange(requestRender);
			} catch { /* optional */ }
	...
	pi.on("session_start", async (_event, ctx: any) => {
		...
		stream = newStream(); lastTPS = null; snap.liveTPS = null; renderAt = 0;
		try { unsubBranch?.(); } catch {}
	...
	pi.on("model_select", async (_event, ctx: any) => { touchCtx(ctx); maybeFetchQuota(); refresh(); });
	...
	pi.on("message_end", async (event: any, ctx: any) => {
		...
		if (stream.messageStartTime != null && msg.stopReason !== "aborted" && msg.stopReason !== "error") {
			const durMs = Date.now() - stream.messageStartTime;
			if (durMs >= MIN_STREAM_MS && typeof (msg.usage as any)?.output === "number" && (msg.usage as any).output > 0) {
				finalRate = ((msg.usage as any).output as number) / (durMs / 1000);
			}
		}
		if (finalRate != null && Number.isFinite(finalRate)) lastTPS = finalRate;
		stream = newStream(); snap.liveTPS = null; // switch display from live estimate snapshot to the accurate provider-reported rate above, once it exists
		requestRender();
	});
	...
	pi.on("message_update", (event: any, ctx: any) => {
		...
		else if (ev.type === "thinking_delta") d = ev.delta; // reasoning tokens are output tokens too (subset of usage.output)
	...
	const cleanup = () => { try { unsubBranch?.(); } catch {} };
