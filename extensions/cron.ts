...
if (runtimeState.__piCronProcessNonce === undefined) {
  runtimeState.__piCronProcessNonce = randomUUID();
}
const PROCESS_NONCE = runtimeState.__piCronProcessNonce;
...
export function parseEvery(value: string): number {
  ...
  if (milliseconds < MIN_EVERY_MS)
    invalid(`repeat interval "${value}" is too small; minimum is 1m.`);
  if (milliseconds > MAX_TIMER_DELAY_MS)
    invalid("repeat intervals cannot exceed approximately 24 days.");
  return milliseconds;
}
...
function relativeTime(milliseconds: number): string {
  ...
  if (seconds < 3_600)
    return `in ${Math.floor(seconds / 60)}m${seconds % 60 ? ` ${seconds % 60}s` : ""}`;
...
function parsePersistedJob(value: unknown): CronJob | undefined {
  ...
  const firedCount =
    typeof value.firedCount === "number" && Number.isFinite(value.firedCount)
      ? Math.max(0, Math.floor(value.firedCount))
      : 0;
  const lastFiredAt =
    typeof value.lastFiredAt === "number" && Number.isFinite(value.lastFiredAt)
      ? value.lastFiredAt
      : undefined;
  return {
    name,
...
const CronParamsSchema = Type.Object({
  ...
  name: Type.Optional(
    Type.String({ description: "Schedule name. Required by add/remove/pause/resume." }),
  ),
  at: Type.Optional(
    Type.String({
      description:
        'One-shot trigger: relative "+30m"/"2h" or ISO timestamp (local time when no zone is given).',
    }),
  ),
  every: Type.Optional(
    Type.String({
  ...
  message: Type.Optional(
    Type.String({ description: "Message delivered when the schedule fires. Required by add." }),
  ),
});
...
export default function cronExtension(pi: ExtensionAPI): void {
  ...
  function deliver(job: CronJob): void {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[cron:${job.name}] could not deliver scheduled wake: ${(error as Error).message}`,
        );
      }
    }
  }
  ...
  function restore(ctx: ExtensionContext): void {
    ...
    if (active.length > 0) {
      ...
      ctx.ui.notify(
        `[cron] restored ${jobs.size} schedule(s); next due ${relativeTime(soonest - Date.now())}.`,
        "info",
      );
    }
  }
      ...
      switch (params.action) {
        case "add": {
          ...
          if (!name || !NAME_RE.test(name))
            invalid(
              "add requires a valid name: start with a letter/digit and use at most 64 letters, digits, spaces, or . _ : -.",
            );
          if (jobs.has(name))
            invalid(`schedule "${name}" already exists; remove it first or choose another name.`);
          if (jobs.size >= MAX_JOBS)
            invalid(`cannot add another schedule; the limit is ${MAX_JOBS}.`);
          ...
          if (message.length > MAX_MESSAGE_LENGTH)
            invalid(`message is too long; maximum is ${MAX_MESSAGE_LENGTH} characters.`);
          ...
          if (hasAt) {
            ...
            if (atMs - now > MAX_TIMER_DELAY_MS)
              invalid("one-shot timestamps cannot be more than approximately 24 days away.");
